...
const els = {
  ...
  latestDocumentStatus: document.getElementById('latestDocumentStatus'),
  latestDocumentType: document.getElementById('latestDocumentType'),
  latestDocumentName: document.getElementById('latestDocumentName'),
  latestDocumentFolder: document.getElementById('latestDocumentFolder'),
...
async function loadEmail() {
  ...
  const emails = Array.isArray(history)
    ? history
    : history.items || [];
...
async function loadCalendar() {
  const overview = await api(API.calendar, '/api/calendar/overview');
  const metrics = overview.metrics || {};
...
async function createCalendarEvent() {
  ...
  const result = await api(API.calendar, '/api/calendar/schedule', {
    method: 'POST',
    body: JSON.stringify(payload),
  });
...
async function loadTasks() {
  const overview = await api(API.task, '/api/tasks/overview');
...
async function loadDocuments() {
  ...
  const overview = await api(API.document, '/api/documents/overview');
  const documents = overview.documents || overview.items || [];

  if (els.metricDocuments) {
    els.metricDocuments.textContent = overview.metrics?.total_documents || documents.length || 0;
  }
  ...
  renderList(
    ...
    (doc) => `
      <article class="item">
        <h5>${escapeHtml(doc.renamed_file || doc.new_filename || doc.original_filename || '(Untitled document)')}</h5>
        <p>${escapeHtml(doc.document_type || 'misc')} · ${escapeHtml(doc.target_folder || 'Needs manual review')}</p>
...
async function fileDocument() {
  const payload = {
    original_filename: els.documentFilename.value.trim() || 'document.pdf',
    ...
    client_name: els.documentClient.value.trim() || 'Unknown Client',
    matter_name: els.documentMatter.value.trim() || 'Unknown Matter',
    document_type: els.documentType.value.trim() || 'misc',
    source: 'unified_dashboard',
  };

  const result = await api(API.document, '/api/documents/file', {
    method: 'POST',
    body: JSON.stringify(payload),
  });

  els.latestDocumentStatus.textContent = result.status || '—';
  els.latestDocumentType.textContent = result.document_type || '—';
  els.latestDocumentName.textContent = result.renamed_file || result.new_filename || '—';
  els.latestDocumentFolder.textContent = result.target_folder || '—';
  els.latestDocumentSummary.textContent = result.summary || 'Document organized.';
...
if (els.documentForm) {
  els.documentForm.addEventListener('submit', async (event) => {
    ...
    try {
      await fileDocument();
      debounceRefresh();
      showAlert('Document organized.', 'success');
    } catch (error) {
      showAlert(error.message);
    }
  });
